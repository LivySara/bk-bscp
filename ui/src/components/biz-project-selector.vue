              ...
              <template v-else>
                ...
                <div
                  ...
                  @click="handleSelectProject($event, item)">
                    <span class="item-name">{{ item.spec.name }}</span>
                  </div>
                </RecycleScroller>
                <div v-else class="column-empty">
...
<script setup lang="ts">
  ...
  import { hasProjectConcept, saveSpaceToProjectId } from '../constants/project';
  ...
  const displayText = computed(() => {
    ...
    const projectList = bizProjectsMap.value[String(selectedBizId.value)] || [];
  ...
  watch(
    ...
    (val) => {
      if (val) {
        ...
        // 如果项目列表未缓存，则主动加载
        if (spaceId.value && !bizProjectsMap.value[String(spaceId.value)]?.length) {
          getProjectList(spaceId.value, { all: true }).then((res) => {
            const projects = res.data?.projects || [];
            bizProjectsMap.value = {
              ...bizProjectsMap.value,
              [String(spaceId.value)]: projects,
            };
          }).catch(() => {
            bizProjectsMap.value = {
              ...bizProjectsMap.value,
              [String(spaceId.value)]: [],
            };
          });
        }
      } else {
  ...
  const handleSelectBiz = async (event: MouseEvent, biz: ISpaceDetail) => {
    ...
    // 如果已缓存该项目列表，直接返回（不重新请求）
    if (bizProjectsMap.value[String(biz.space_id)]?.length > 0) {
      return;
    }
