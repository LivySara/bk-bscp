            ...
            <template #default="{ row }">
              <div v-if="row.app_id" class="app-info">
                ...
                <div
                  ...
                  class="env-tag"
                  :style="{
                    backgroundColor: (ENV_TYPE_CONFIG as any)[row.env_type]?.bgColor || '#F5F7FA',
                    color: (ENV_TYPE_CONFIG as any)[row.env_type]?.textColor || '#63656E',
                  }">
                  <i
                    :class="`bk-bscp-icon ${(ENV_TYPE_CONFIG as any)[row.env_type]?.iconClass || ''} env-icon`"
                    :style="{ color: (ENV_TYPE_CONFIG as any)[row.env_type]?.iconColor || '#979BA5' }"></i>
                  {{ row.env_name }}
                </div>
              </div>
            </template>
