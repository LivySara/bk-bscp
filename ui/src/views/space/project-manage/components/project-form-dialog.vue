  ...
  <bk-dialog
    ...
    <bk-form ref="formRef" form-type="vertical" :model="formData" :rules="formRules" class="project-form">
      <bk-form-item :label="t('项目名称')" property="name" required>
        <bk-input v-model="formData.name" :placeholder="t('请输入项目名称')" :maxlength="32" />
      </bk-form-item>
      <bk-form-item :label="t('项目描述')" property="description" required>
        <bk-input
...
<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEdit.value) {
        await updateProject(spaceId.value, props.editingItem.id!, formData);
